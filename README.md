...
Используется ConfigMap `autoscaler-props` в пространстве имен `unip-system-autoscaler`.

По ключу `spec.yaml` указывается содержимое YAML-файла с конфигурацией.

Между собой конфигурации отделяются разделителем `---` в `spec.yaml`.
